...
def summarize_patdata(ids__uid, wlen_min=10, cfg_fname="cfg/db.cfg", verbose=0, cache_dir="cache"):
    ...
    out_fname = os.path.join(cache_dir, ids__uid + "_" + str(wlen_min)+"min.pklz")
    if not os.path.exists(out_fname):
        ...
        n_pos_frames_per_evt_signal = {evt: 
                                            {thesig: X[evt][frames_with_features_per_signal[thesig].values].sum() for thesig in ["spo2","rf","btb"] } 
        ...
        a = [{thesig + "__" + evt : value for thesig, value in n_pos_frames_per_evt_signal[evt].items()} for evt in n_pos_frames_per_evt_signal.keys()]+[n_ctrl_frames_per_signal]
...
if __name__ == "__main__":
    ...
    query = "select ov.ids__uid as \"ids__uid2\", ov.bw, ov.ga_w,vuh.* from view__uid_has vuh, overview ov\
        where vuh.sepsis=1 and neo=1 and takecare=1 and ((spo2=1)or(btb=1)) and vuh.ids__uid =ov.ids__uid "
