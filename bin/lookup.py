...
def export_raw_lf_data(ids__uid, outdir=".", verbose=0, _read=False, _return=False):
    ...
    if not os.path.exists(outfname):
        ...
        if len(the_intervals) >0:
            ...
            # Export
            dout = pd.concat(list(allsigs.values()), axis=1)
        else:
            if verbose:
...
if __name__ == "__main__":
    ...
    if parameter_data:
        ...
        out = list(map(partial(export_raw_lf_data, outdir=outdir, verbose=verbose,_return=True,_read=False), study_pat_list))
